<template>
  <div class="text-left p-4 rounded-lg bg-slate-300">
    ...
    <div
      class="relative shadow-md shadow-slate-500/50 max-w-lg rounded-md bg-slate-200 mx-auto space-y-2 p-4 sm:p-6"
    >
      <div
        class="max-w-lg flex items-center justify-center space-x-1"
        v-for="(i, row) in board"
        :key="i"
      >
        <div
          class="text-white text-2xl font-bold inline-block w-40 aspect-square rounded sm:rounded-md flex items-center justify-center bg-slate-500 shadow-lg"
          v-for="(j, col) in i"
          :key="j"
    ...
    <div class="text-center mt-4">
      <button
        class="text-sm sm:text-base shadow-md transition-colors px-6 py-2 rounded-md bg-slate-500 hover:bg-slate-400 active:bg-slate-400 text-white"
